...
- **Direct-to-Google uploads (no Vercel size cap):** photos upload **one file per request, at full resolution, straight from the guest's browser to the Apps Script Web App** — bypassing Vercel's ~4.5 MB body limit entirely. Guests can select 50+ at a time; uploads run 3 at a time with a per-photo progress bar and auto-retry. Nothing is resized unless a fallback is needed (see below).
  - How the secret stays safe: the browser never holds `UPLOAD_TOKEN`. Instead `/api/upload-ticket` mints a short-lived (2 h) **signed ticket** (HMAC of an expiry, keyed by `UPLOAD_TOKEN`), and the Apps Script verifies it. No new environment variables are required — the ticket reuses the token both sides already share. **You must redeploy the Apps Script** (Manage deployments → new version) after updating `Code.gs`, or ticket verification will fail.
  - Automatic fallback: if the direct upload is ever blocked (e.g. a browser/CORS quirk), files **≤ ~4.3 MB** fall back to the old `/api/upload` server-proxy path, resizing the image only if needed to fit under Vercel's cap.
- **Videos & very large files:** the direct path handles files up to ~30 MB (`DIRECT_MAX_BYTES` in `app/lib/uploader.ts`) — every photo and short phone clips. Larger files exceed Apps Script's own POST limit and are flagged "too large to upload"; share those another way (e.g. a shared Drive/Photos link). True unlimited large-video support needs the resumable Drive API (OAuth), which is a bigger setup.
