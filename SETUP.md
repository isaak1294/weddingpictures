...
- **Upload size:** photos upload **one file per request** at full quality. A photo is only resized in the guest's browser when its original exceeds Vercel's ~4.5 MB body cap, and then only as much as needed to fit (`MAX_UPLOAD_BYTES` / `COMPRESS_TARGET_BYTES` in `app/lib/uploader.ts`). Guests can select 50+ at a time; uploads run 3 at a time with a per-photo progress bar and auto-retry.
